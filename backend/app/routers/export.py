...
def _check_completeness(p: Process) -> tuple[bool, list[str]]:
    ...
    if p.rto_rpo is None:       missing.append("RTO / RPO")
...
def _build_xlsx_dashboard(db: Session, sections: list[str]) -> io.BytesIO:
            ...
            for p in processes:
                if p.bia is None:
                    ...
                    if score in counts:
                        ...
                        not_assessed += 1
            ws.append([dim, counts[1], counts[2], counts[3], counts[4], counts[5], not_assessed])
        _xl_autowidth(ws)
    ...
    if "review" in sections:
        ...
        for cat, (done, total_n, pct) in rev.items():
            ws.append([cat, done, total_n, f"{pct}%"])
        _xl_autowidth(ws)
    ...
    if "acties" in sections:
        ...
        for a in _get_priority_actions(processes):
            ws.append([a["code"], a["naam"], a["kritiek"], a["prioriteit"], a["reden"], a["ontbrekend"]])
        _xl_autowidth(ws)
...
def _build_xlsx_processes(db: Session, sections: list[str]) -> io.BytesIO:
    ...
    if "basis" in sections or "details" in sections or "biv" in sections or "rto_rpo" in sections or "applicaties" in sections or "datums" in sections:
        ...
        if "rto_rpo" in sections:
            headers += ["RTO waarde", "RTO eenheid", "RPO waarde", "RPO eenheid"]
        ...
        for p in processes:
            ...
            if "rto_rpo" in sections:
                row += [
                    float(p.rto_rpo.rto_value) if p.rto_rpo and p.rto_rpo.rto_value else "",
                    p.rto_rpo.rto_unit if p.rto_rpo else "",
                    float(p.rto_rpo.rpo_value) if p.rto_rpo and p.rto_rpo.rpo_value else "",
...
def _build_xlsx_bia(db: Session, sections: list[str]) -> io.BytesIO:
    ...
    if "vertrouwelijkheid" in sections:
        ...
        q_headers = ["Proces"] + [label for _, label in V_QUESTIONS] + ["Eindcore V"]
        _xl_header(ws, q_headers)
        for p in processes:
            if p.bia is None:
                continue
...
def _build_xlsx_ketenarchitectuur(db: Session, sections: list[str]) -> io.BytesIO:
    ...
    if "applicaties" in sections:
        ...
        for a in applications:
            ws.append([a.code, a.name, a.business_owner or "", a.technical_owner or "", len(a.processes)])
        _xl_autowidth(ws)
    ...
    if "koppelingen" in sections:
        ...
        for p in processes:
            for a in p.applications:
                ws.append([p.code, p.name, a.code, a.name])
        _xl_autowidth(ws)
...
def _build_docx_processes(db: Session, sections: list[str]) -> io.BytesIO:
    ...
    if "rto_rpo" in sections:
        ...
        rows = []
        for p in processes:
            rto = f"{p.rto_rpo.rto_value} {p.rto_rpo.rto_unit}" if p.rto_rpo and p.rto_rpo.rto_value else "N/A"
            rpo = f"{p.rto_rpo.rpo_value} {p.rto_rpo.rpo_unit}" if p.rto_rpo and p.rto_rpo.rpo_value else "N/A"
            rows.append([p.code, p.name, rto, rpo])
...
def _build_docx_bia(db: Session, sections: list[str]) -> io.BytesIO:
    ...
    if "beschikbaarheid" in sections:
        ...
        headers = ["Proces"] + [label for _, label in B_QUESTIONS] + ["Eindcore B"]
        rows = []
        for p in processes:
            if p.bia is None:
                continue
    ...
    if "integriteit" in sections:
        ...
        headers = ["Proces"] + [label for _, label in I_QUESTIONS] + ["Eindcore I"]
        rows = []
        for p in processes:
            if p.bia is None:
                continue
    ...
    if "vertrouwelijkheid" in sections:
        ...
        headers = ["Proces"] + [label for _, label in V_QUESTIONS] + ["Eindcore V"]
        rows = []
        for p in processes:
            if p.bia is None:
                continue
